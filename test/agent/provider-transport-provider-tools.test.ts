...
function createFlakyObservingPlatformBridge(): {
	bridge: PlatformToolExecutionBridge;
	recordObservation: ReturnType<typeof vi.fn>;
} {
	const recordObservation = vi.fn(async (_plan, result) => {
		if (recordObservation.mock.calls.length >= 2) {
...
describe("ProviderTransport provider-owned tool events", () => {
	...
	it("fails open when cached dynamic read observation recording errors", async () => {
		const { bridge, recordObservation } = createFlakyObservingPlatformBridge();
		const toolExecute = vi.fn(async (_toolCallId, params) => ({
			content: [
				{
					type: "text" as const,
		...
		mocks.createProviderStream.mockImplementationOnce(async function* (
			...
			for (let index = 1; index <= 2; index++) {
				const toolCallId = `dynamic-flaky-read-${index}`;
				const result = await options.executeDynamicTool({
					type: "toolCall",
					id: toolCallId,
					name: "read",
		...
		await withTimeout(
			...
			"Timed out waiting for flaky observed Codex dynamic reads",
		);
		...
		expect(readResults.map((event) => event.isError)).toEqual([false, false]);
		expect(
		...
		).toEqual(["dynamic:/tmp/evalops.txt:1", "dynamic:/tmp/evalops.txt:1"]);
	});
	...
	it("fails open when cached provider read observation recording errors", async () => {
		const { bridge, recordObservation } = createFlakyObservingPlatformBridge();
		const toolExecute = vi.fn(async (_toolCallId, params) => ({
			content: [
				{
		...
		mocks.createProviderStream.mockImplementation(async function* () {
			...
			if (streamCount <= 2) {
				yield {
					type: "toolcall_end",
					toolCall: {
						id: `flaky-read-call-${streamCount}`,
						name: "read",
						arguments: { file_path: "/tmp/evalops.txt" },
					},
					partial: assistant,
		...
		expect(readResults.map((event) => event.isError)).toEqual([false, false]);
		expect(readResults.map((event) => event.toolExecutionId)).toEqual([
			"observed:flaky-read-call-1",
			"prepared:flaky-read-call-2",
		]);
	});
