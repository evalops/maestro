...
describe("A2A capability market", () => {
	...
	it("matches omitted skill requests against later advertised skills", () => {
		const multiSkillPeer = candidate("agent-multi-skill", {
			skills: [
				skill("maestro.subagent.refactor", {
					allowedTaskClasses: ["code.refactor"],
					requiredArtifactKinds: ["patch.diff"],
					requiredContextGrants: ["repo:read"],
				}),
				skill("maestro.subagent.review", {
					allowedTaskClasses: ["code.review"],
					requiredArtifactKinds: ["review.summary"],
		...
		const rank = selectA2ACapabilityPeer([multiSkillPeer], {
			taskClass: "code.review",
			requiredArtifactKinds: ["review.summary"],
			requiredContextGrants: ["repo:read", "issue:read"],
			now: NOW,
	...
	it("filters peers with no selectable skill for task-class routing", () => {
		const noSkills = candidate("agent-no-skills", {
			skills: [],
		});
		const wrongSkill = candidate("agent-wrong-skill", {
			skills: [
				skill("maestro.subagent.refactor", {
					allowedTaskClasses: ["code.refactor"],
				}),
		...
		expect(
			rankA2ACapabilityPeers([noSkills, wrongSkill], {
				taskClass: "code.review",
				now: NOW,
			}),
		).toEqual([]);
	...
	it("trims required context grants and artifacts before matching", () => {
		const compatible = candidate("agent-compatible", {
			skills: [
				skill("maestro.subagent.code-review", {
					requiredContextGrants: ["repo:read", "issue:read"],
		...
		expect(
			selectA2ACapabilityPeer([compatible], {
				skillId: "maestro.subagent.code-review",
				requiredContextGrants: [" repo:read", "issue:read "],
				requiredArtifactKinds: [" review.summary "],
				now: NOW,
