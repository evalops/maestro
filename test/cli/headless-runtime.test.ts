...
describe("runHeadlessMode", () => {
	...
	it("rejects viewer cancel requests before aborting the local session", async () => {
		let onLine: LineHandler | undefined;
		let onClose: CloseHandler | undefined;
		const readlineInterface = {
			on(event: string, handler: LineHandler | CloseHandler) {
		...
		await onLine?.(JSON.stringify({ type: "cancel" }));
		onClose?.();
		await runPromise;
	...
	it("rejects viewer client_tool_result messages before handling them", async () => {
		let onLine: LineHandler | undefined;
		let onClose: CloseHandler | undefined;
		const readlineInterface = {
			on(event: string, handler: LineHandler | CloseHandler) {
		...
		expect(
			messages.some(
				(message) =>
					message.type === "error" &&
					typeof message.message === "string" &&
					message.message.includes("No pending client tool request found"),
			),
		).toBe(false);
	});
