...
const ORIGINAL = {
	shimmer: process.env.MAESTRO_TUI_SHIMMER,
	noColor: process.env.NO_COLOR,
	composerNoColor: process.env.MAESTRO_NO_COLOR,
	forceColor: process.env.FORCE_COLOR,
};

function resetEnv() {
	process.env.MAESTRO_TUI_SHIMMER = ORIGINAL.shimmer;
	process.env.NO_COLOR = ORIGINAL.noColor;
	process.env.MAESTRO_NO_COLOR = ORIGINAL.maestroNoColor;
...
describe("footer stage shimmer", () => {
	...
	it("keeps colored badge when shimmer is off", () => {
		process.env.MAESTRO_TUI_SHIMMER = "off";
	...
	it("adds more ANSI escapes when shimmer is on", () => {
		process.env.MAESTRO_TUI_SHIMMER = "on";
		Reflect.deleteProperty(process.env, "NO_COLOR");
		Reflect.deleteProperty(process.env, "MAESTRO_NO_COLOR");
		process.env.FORCE_COLOR = "1";
