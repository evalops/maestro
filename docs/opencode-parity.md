...
## Status Snapshot
- ✅ Plan Mode (ask-before-write/bash): `/plan-mode on|off` + `COMPOSER_PLAN_MODE=1` gates `write/edit/bash` in the safety firewall and surfaces status in `/status`.
- ✅ User Command Catalog: JSON commands in `~/.composer/commands/*.json` or `.composer/commands/`, exposed via `/commands list|run` and the command palette.
...
- ✅ LSP Auto-Attach: `COMPOSER_LSP_AUTOSTART=1` triggers `src/lsp/autostart.ts` detection and `/lsp status|restart`.
...
- **Implementation sketch:**
  - Add `COMPOSER_PROFILE=curated` to load a curated models allowlist + caps.
