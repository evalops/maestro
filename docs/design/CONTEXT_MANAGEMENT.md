...
> **Status:** This document predates the Rust-only runtime migration (#3016, #3017, merged 2026-07-22), which deleted Maestro's TypeScript agent runtime and SDK. Token estimation, model-aware counting, and prepared-request usage accounting live in `packages/context-rs`; context assembly and compaction remain in `packages/tui-rs/src/agent/` (see `compaction.rs`, `message_queue.rs`). Some file paths below may be stale; they are kept for design context and updated only where a corresponding Rust module was confirmed.
