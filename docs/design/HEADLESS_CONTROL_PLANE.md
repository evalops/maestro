...
> **Status:** This document predates the Rust-only runtime migration (#3016, #3017, merged 2026-07-22), which deleted Maestro's TypeScript agent runtime and SDK. Headless/remote runtime code now lives in `packages/tui-rs/src/headless/` and `packages/control-plane-rs`. Some file paths below may be stale; they are kept for design context and updated only where a corresponding Rust module was confirmed.
