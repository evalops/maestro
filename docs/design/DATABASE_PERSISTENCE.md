...
> **Status:** This document predates the Rust-only runtime migration (#3016, #3017, merged 2026-07-22), which deleted Maestro's TypeScript agent runtime and SDK. Database/persistence code now lives in `packages/control-plane-rs/src/migrations.rs` and `sessions.rs`. Some file paths below may be stale; they are kept for design context and updated only where a corresponding Rust module was confirmed.
