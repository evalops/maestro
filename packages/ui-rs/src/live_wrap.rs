...
//!
//! This module provides utilities for wrapping text incrementally as it arrives,
//! which is essential for rendering streaming AI responses in real-time.
//!
//! Ported from OpenAI Codex CLI (MIT licensed).
//!
//! # Key Features
//!
//! - **Incremental Wrapping**: Text can be pushed in fragments as it streams in.
//!   The builder maintains state and produces visual rows correctly regardless
//!   of how the input is chunked.
//!
//! - **Fragmentation Invariance**: The output is the same whether you push "hello world"
//!   all at once or as "hel", "lo wor", "ld". This is critical for streaming scenarios.
//!
//! - **Unicode Width Aware**: Uses `unicode_width` for proper handling of CJK characters,
//!   emojis, and other wide characters.
//!
//! # Example
//!
//! ```rust
//! use maestro_ui::live_wrap::RowBuilder;
//!
//! let mut rb = RowBuilder::new(40);
//! rb.push_fragment("Hello ");
//! rb.push_fragment("world! This is a long line that wraps.");
//! rb.end_line();
//!
...
///
/// This builder handles streaming input correctly - you can push text in arbitrary
/// chunks and the output will be the same as if you had pushed all the text at once.
...
impl RowBuilder {
    ...
    /// Update the target width and rewrap all content
    pub fn set_width(&mut self, width: usize) {
        self.target_width = width.max(1);
        // Rewrap everything we have (simple approach)
...
mod tests {
    ...
    fn rows_do_not_exceed_width_emoji_cjk() {
        // U+1F600 is width 2; CJK chars are width 2
