...
///
/// Inserts an ellipsis in the middle when the path is too long.
///
/// # Arguments
///
...
pub fn center_truncate_path(path: &str, max_width: usize) -> String {
    ...
    // Simple approach: keep first and last parts with ellipsis
    let sep = std::path::MAIN_SEPARATOR;
    let parts: Vec<&str> = path.split(sep).collect();

    if parts.len() <= 2 {
        // Can't split further, just truncate from end
        return truncate_text(path, max_width);
    }

    // Try keeping first and last segment
    let first = parts.first().unwrap_or(&"");
    let last = parts.last().unwrap_or(&"");
    let middle = format!("{}…{}{}", first, sep, last);
...
mod tests {
    ...
    fn test_center_truncate_path_long() {
        let path = "/home/user/very/long/path/to/some/file.txt";
        let result = center_truncate_path(path, 20);
