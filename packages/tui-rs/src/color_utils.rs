...
//!
//! Provides functions for color manipulation and matching:
//! - Perceptual color distance using CIE76 formula
//! - Color blending with alpha
//! - Light/dark background detection
//! - Best color matching for terminal capabilities
//!
...
/// * `fg` - Foreground color
/// * `bg` - Background color
/// * `alpha` - Blend ratio (0.0 = bg, 1.0 = fg)
///
/// # Returns
///
...
///
/// Uses the CIE76 formula (Euclidean distance in Lab space).
/// This gives better results than simple RGB distance because it accounts
/// for how humans perceive color differences.
///
/// # Returns
///
/// A distance value where smaller = more similar. Typical values:
...
/// Find the best matching color for the terminal's capabilities.
///
...
///
/// Checks multiple signals to work across local and SSH sessions:
...
pub fn has_true_color_support() -> bool {
    ...
    if let Ok(term) = std::env::var("TERM") {
        ...
        if t.contains("24bit") || t.contains("truecolor") || t.contains("direct") {
            return true;
        }
        // Modern terminals that support true color
        if t.starts_with("xterm-direct")
            || t.starts_with("iterm2")
            || t == "xterm-kitty"
            || t == "alacritty"
        {
            return true;
    ...
    // Check for specific terminal emulators via their env vars
    // These work even over SSH when the terminal sets them
...
pub fn has_256_color_support() -> bool {
    ...
    if let Ok(term) = std::env::var("TERM") {
        ...
        // xterm and most modern terminals support 256 colors
...
/// The subset of xterm colors that are consistent across terminals (indices 16-255).
fn xterm_fixed_colors() -> impl Iterator<Item = (usize, (u8, u8, u8))> {
    XTERM_COLORS.into_iter().enumerate().skip(16)
}

/// Xterm 256 color palette.
/// The first 16 colors vary by terminal theme, so we skip them when matching.
/// Colors 16-255 are standardized.
pub const XTERM_COLORS: [(u8, u8, u8); 256] = [
    // System colors 0-15 (vary by terminal theme)
    (0, 0, 0),       //   0 Black
    (128, 0, 0),     //   1 Maroon
    (0, 128, 0),     //   2 Green
    (128, 128, 0),   //   3 Olive
    ...
    // Standard colors 16-231 (6x6x6 color cube)
    (0, 0, 0),       //  16 Grey0
    (0, 0, 95),      //  17 NavyBlue
    (0, 0, 135),     //  18 DarkBlue
    (0, 0, 175),     //  19 Blue3
...
mod tests {
    ...
    fn test_is_light_gray() {
        // Gray 128 should be at the boundary
        assert!(!is_light((128, 128, 128)));
        assert!(is_light((130, 130, 130)));
    }

    #[test]
    fn test_blend_full_fg() {
        let result = blend((255, 0, 0), (0, 0, 255), 1.0);
        assert_eq!(result, (255, 0, 0));
    }

    #[test]
    fn test_blend_full_bg() {
    ...
    fn test_perceptual_distance_identical() {
        let dist = perceptual_distance((100, 150, 200), (100, 150, 200));
        assert!(dist < 0.001);
    }

    #[test]
    fn test_perceptual_distance_different() {
        let dist = perceptual_distance((0, 0, 0), (255, 255, 255));
        // Black to white should be a large distance
        assert!(dist > 50.0);
    }

    #[test]
    fn test_perceptual_distance_similar() {
        let dist = perceptual_distance((100, 100, 100), (105, 100, 100));
        // Very similar colors should have small distance
        assert!(dist < 5.0);
    }

    #[test]
    fn test_xterm_colors_len() {
        assert_eq!(XTERM_COLORS.len(), 256);
    }
}
