//! TUI compatibility facade for the native Maestro runtime.
//!
...
//! concrete TUI execution host around that actor.
...
use crate::agent::native_host::TuiNativeExecutionHost;
...
fn build_tui_host(
    ...
    Ok(TuiNativeExecutionHost::compose(
        Arc::new(executor),
        hooks,
        resolve_model,
        model_route,
...
fn relay_runtime_events(
    ...
    let mut turn_tracker =
        crate::telemetry::TurnTracker::new(crate::telemetry::TurnTrackerConfig {
            session_id: managed_run_id.to_owned(),
...
#[cfg(test)]
pub(crate) use self::native_host::TuiNativeExecutionHost as TestTuiNativeExecutionHost;
