...
impl HarnessStore {
    ...
    pub fn report(&self, workspace: &Path, session_id: Option<&str>) -> String {
        ...
        let path = self
            ...
            .unwrap_or_else(|| maestro_ui::localization::tr("(in memory)").to_string());
        let mut report = maestro_ui::localization::format(
            ...
            &[
                ...
                (self.proposals.len()).to_string(),
                (self.history.len()).to_string(),
                (self.snapshots.len()).to_string(),
            ],
        );
        if visible.is_empty() {
            report.push_str(maestro_ui::localization::tr(
        ...
        report.push_str(maestro_ui::localization::tr("Use `/refine propose <scope> <kind> <name> <content> --evidence <text>` to stage a reviewed change.\n"));
        report.push_str(maestro_ui::localization::tr(
    ...
    pub fn list_report(&self, workspace: &Path, session_id: Option<&str>) -> String {
        let entries = self.visible_entries(workspace, session_id);
        if entries.is_empty() {
            return maestro_ui::localization::tr(
                "No harness entries match the current workspace or session.",
            )
            .to_string();
        }
        let mut report = String::from(maestro_ui::localization::tr("## Harness entries\n\n"));
        for entry in entries {
            let evidence = entry
                .evidence
                .as_deref()
                .map(|value| {
                    maestro_ui::localization::format("\n  Evidence: {0}", &[(value).to_string()])
                })
                .unwrap_or_default();
            report.push_str(&format!(
                "- `{}` · {} · {} · {}\n  {}{}\n",
    ...
    pub fn proposal_report(&self) -> String {
        if self.proposals.is_empty() {
            return maestro_ui::localization::tr("No harness refinement proposals.").to_string();
        }
        let mut report = String::from(maestro_ui::localization::tr(
            "## Harness refinement proposals\n\n",
        ));
        for proposal in self.proposals.iter().rev() {
            report.push_str(&maestro_ui::localization::format(
                "- `{0}` · {1} · {2} · {3} · {4}\n  {5}\n  Evidence: {6}\n",
                &[
                    (proposal.id).clone(),
                    (match proposal.status {
                        HarnessProposalStatus::Pending => maestro_ui::localization::tr("pending"),
                        HarnessProposalStatus::Applied => maestro_ui::localization::tr("applied"),
                        HarnessProposalStatus::Rejected => maestro_ui::localization::tr("rejected"),
                    })
                    .to_string(),
