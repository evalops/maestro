...
impl GoalStore {
    ...
    pub fn report(&self) -> String {
        match &self.current {
            None => maestro_ui::localization::tr(
                "No active goal. Create one with `/goal create <text>`.",
            )
            .to_string(),
            Some(g) => {
                let budget_line = match g.token_budget {
                    Some(b) => maestro_ui::localization::format(
                        "**Tokens:** {0} / {1}\n",
                        &[(g.tokens_used).to_string(), (b).to_string()],
                    ),
                    None => maestro_ui::localization::format(
                        "**Tokens used:** {0}\n",
                        &[(g.tokens_used).to_string()],
                    ),
                };
                let mut out = maestro_ui::localization::format(
                    "## Goal {0}\n\n**Status:** {1}\n**Auto-continue:** {2} ({3} turns; safety max {4})\n{5}**Completion:** worker calls `update_goal` complete|blocked (same model; Codex-style)\n\n{6}\n",
                    &[
                        (g.id).clone(),
                        maestro_ui::localization::tr(g.status.as_str()).to_string(),
                        (if g.auto_continue {
                            maestro_ui::localization::tr("on")
                        } else {
                            maestro_ui::localization::tr("off")
                        })
                        .to_string(),
                        (g.auto_continue_count).to_string(),
                        (g.max_turns).to_string(),
                        (budget_line).clone(),
                        (g.text).clone(),
                    ],
                );
                ...
                if let Some(max_duration) = g.max_duration_secs {
                    ...
                    out.push_str(&maestro_ui::localization::format(
                        ...
                        &[(elapsed).to_string(), (max_duration).to_string()],
                    ));
                }
                out.push_str(
