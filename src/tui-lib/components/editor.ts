...
export class Editor implements Component {
    ...
    private layoutText(contentWidth: number): Array<{ text: string; hasCursor: boolean; cursorPos?: number }> {
        const layoutLines = [];
    ...
    tryTriggerAutocomplete(explicitTab = false) {
        ...
        if (explicitTab) {
            ...
            const shouldTrigger = !("shouldTriggerFileCompletion" in provider) ||
                (provider as any).shouldTriggerFileCompletion(this.state.lines, this.state.cursorLine, this.state.cursorCol);
            if (!shouldTrigger) {
                return;
            }
        }
    ...
    private forceFileAutocomplete(): void {
        ...
        const suggestions = (provider as any).getForceFileSuggestions(this.state.lines, this.state.cursorLine, this.state.cursorCol);
        if (suggestions && suggestions.items.length > 0) {
            this.autocompletePrefix = suggestions.prefix;
            this.autocompleteList = new SelectList(suggestions.items, 5);
            this.isAutocompleting = true;
