...
// Manual overlay for Databricks serving endpoints exposed through an
// OpenAI-compatible chat completions surface.
const DATABRICKS_OVERLAY = {
	databricks: {
		"databricks-meta-llama-3-3-70b-instruct": {
			...
			api: "openai-completions",
			provider: "databricks",
			baseUrl: "https://dbc.databricks.com",
			reasoning: false,
			toolUse: true,
			input: ["text"],
			cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
		...
		"databricks-code-llama-34b-instruct": {
			id: "databricks-code-llama-34b-instruct",
			name: "Databricks Code Llama 34B Instruct",
			api: "openai-completions",
			provider: "databricks",
			baseUrl: "https://dbc.databricks.com",
			reasoning: false,
		...
		"databricks-code-llama-13b-instruct": {
			id: "databricks-code-llama-13b-instruct",
			name: "Databricks Code Llama 13B Instruct",
			api: "openai-completions",
			provider: "databricks",
			baseUrl: "https://dbc.databricks.com",
			reasoning: false,
...
// Endpoint: https://api.deepseek.com (OpenAI-compatible; also exposes an
// Anthropic-compatible surface at https://api.deepseek.com/anthropic).
// `deepseek-chat` (non-thinking) and `deepseek-reasoner` (thinking) are the
// canonical, always-current model aliases that DeepSeek keeps pointed at the
// latest V-series weights. Auth: DEEPSEEK_API_KEY.
const DEEPSEEK_OVERLAY = {
	deepseek: {
		...
		"deepseek-v3.2": {
			...
			toolUse: true,
			input: ["text"],
			cost: { input: 0.27, output: 0.41, cacheRead: 0.028, cacheWrite: 0 },
...
// Manual overlay for Moonshot AI (Kimi) models (OpenAI-compatible API).
// Endpoint: https://api.moonshot.ai/v1 (international; the China-mainland
// endpoint is https://api.moonshot.cn/v1). Moonshot also exposes an
// Anthropic-compatible surface at https://api.moonshot.ai/anthropic.
// Auth: MOONSHOT_API_KEY (KIMI_API_KEY is accepted as an alias).
const MOONSHOT_OVERLAY = {
	moonshot: {
		...
		"moonshot-v1-128k": {
			...
			toolUse: true,
			input: ["text"],
			cost: { input: 2.0, output: 5.0, cacheRead: 0, cacheWrite: 0 },
			contextWindow: 131072,
			maxTokens: 16384,
		} as Model<"openai-completions">,
	},
...
// Manual overlay for Alibaba Qwen models via DashScope (OpenAI-compatible API).
// Endpoint: https://dashscope-intl.aliyuncs.com/compatible-mode/v1
// (international/Singapore; China-mainland is
// https://dashscope.aliyuncs.com/compatible-mode/v1).
// Auth: DASHSCOPE_API_KEY (QWEN_API_KEY accepted as an alias).
const DASHSCOPE_OVERLAY = {
	dashscope: {
		"qwen3-max": {
			id: "qwen3-max",
			name: "Qwen3 Max",
			...
			toolUse: true,
			input: ["text"],
			cost: { input: 1.2, output: 6.0, cacheRead: 0.24, cacheWrite: 0 },
			contextWindow: 262144,
			maxTokens: 32768,
		} as Model<"openai-completions">,
		"qwen-max": {
			...
			toolUse: true,
			input: ["text"],
			cost: { input: 1.6, output: 6.4, cacheRead: 0, cacheWrite: 0 },
			contextWindow: 32768,
			maxTokens: 8192,
		} as Model<"openai-completions">,
		"qwen-plus": {
			...
			toolUse: true,
			input: ["text"],
			cost: { input: 0.4, output: 1.2, cacheRead: 0.16, cacheWrite: 0 },
			contextWindow: 131072,
			maxTokens: 8192,
		} as Model<"openai-completions">,
		"qwen-turbo": {
...
// Manual overlay for MiniMax models (OpenAI-compatible API).
// Endpoint: https://api.minimax.io/v1 (international; China-mainland is
// https://api.minimaxi.com/v1). MiniMax also exposes an Anthropic-compatible
// surface at https://api.minimax.io/anthropic. Auth: MINIMAX_API_KEY.
const MINIMAX_OVERLAY = {
	minimax: {
		"MiniMax-M2": {
			id: "MiniMax-M2",
			name: "MiniMax M2",
...
const TOGETHER_OVERLAY = {
	together: {
		...
		"Qwen/Qwen2.5-VL-72B-Instruct": {
			...
			name: "Together Qwen 2.5 VL 72B Instruct",
			api: "openai-completions",
			provider: "together",
			baseUrl: "https://api.together.xyz/v1",
			reasoning: false,
