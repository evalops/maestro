/**
 * Run reconstruction report builders for fixture parity and residual TS callers.
 *
 * The `maestro run` CLI entrypoint is implemented in Rust
 * (`packages/tui-rs/src/run_cli.rs`) and dispatched via
 * `NATIVE_UTILITY_COMMANDS`. This module owns the pure report shape used by
 * trajectory fixture scripts and unit tests.
 */
...
} from "./agent-runtime-ledger.js";
import {
	type AgentTrajectoryInspectionReport,
	buildAgentTrajectoryInspectionReport,
} from "./agent-trajectory-inspection.js";
import { DEFAULT_AGENT_TRAJECTORY_REPLAY_LAB_RULES } from "./agent-trajectory-replay-lab.js";
import {
...
export interface RunReconstructionReport {
...
export async function buildRunReconstructionReport(
	sessionId: string,
...
export function renderRunReconstruction(
	report: RunReconstructionReport,
): string {
	const lines = [
