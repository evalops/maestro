...
// ============================================================================
// USAGE ANALYTICS (LLM calls aggregated by workspace/agent/day)
// ============================================================================
...
export const usageMetrics = pgTable(
	...
	{
		id: uuid("id").primaryKey().defaultRandom(),
		/** Stable workspace identifier; defaults to the server cwd when not provided by a client. */
		workspaceId: varchar("workspace_id", { length: 255 }).notNull(),
		/** Stable agent identifier; web requests use the active Maestro session id. */
		agentId: varchar("agent_id", { length: 255 }).notNull(),
		/** Most recent session id contributing to this aggregate bucket. */
		...
		totalTokens: bigint("total_tokens", { mode: "number" })
			.default(0)
			.notNull(),
		/** Cost in USD micros to avoid floating point drift in aggregates. */
	...
	(table) => ({
		dimensionIdx: uniqueIndex("usage_metric_dimension_idx").on(
			table.workspaceId,
			table.agentId,
			table.provider,
			table.model,
		...
		workspaceBucketIdx: index("usage_metric_workspace_bucket_idx").on(
			table.workspaceId,
			table.bucketStart,
		),
		agentBucketIdx: index("usage_metric_agent_bucket_idx").on(
			table.agentId,
			table.bucketStart,
		),
		modelBucketIdx: index("usage_metric_model_bucket_idx").on(
			table.provider,
			table.model,
			table.bucketStart,
		),
...
// ============================================================================
// EXECUTION TRACES (agent execution observability)
// ============================================================================

export const executionTraces = pgTable(
	"execution_traces",
	{
		/** OpenTelemetry-compatible trace id when generated by Maestro. */
	...
	(table) => ({
		workspaceCreatedIdx: index("execution_trace_workspace_created_idx").on(
			table.workspaceId,
			table.createdAt,
		),
		agentCreatedIdx: index("execution_trace_agent_created_idx").on(
			table.agentId,
			table.createdAt,
		),
		statusCreatedIdx: index("execution_trace_status_created_idx").on(
			table.status,
			table.createdAt,
		),
	}),
...
// ============================================================================
// WORKSPACE CONFIGURATION (workspace-scoped policy and runtime settings)
// ============================================================================
...
export const workspaceConfig = pgTable(
	"workspace_config",
	{
		...
		modelPreferences: jsonb("model_preferences")
			...
			.default({})
			.notNull(),
		safetyRules: jsonb("safety_rules").$type<unknown>().default({}).notNull(),
		rateLimits: jsonb("rate_limits").$type<unknown>().default({}).notNull(),
		createdAt: timestamp("created_at", { withTimezone: true })
			.defaultNow()
			.notNull(),
		updatedAt: timestamp("updated_at", { withTimezone: true })
	...
	(table) => ({
		updatedIdx: index("workspace_config_updated_idx").on(table.updatedAt),
	}),
);

// ============================================================================
// REVENUE ATTRIBUTION (agent actions mapped to business outcomes)
// ============================================================================

export const revenueAttribution = pgTable(
	"revenue_attribution",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		workspaceId: varchar("workspace_id", { length: 255 }).notNull(),
		...
		outcomeId: varchar("outcome_id", { length: 255 }).notNull(),
		outcomeType: varchar("outcome_type", { length: 100 }).notNull(),
		attributionModel: varchar("attribution_model", { length: 50 })
			.default("direct")
			.notNull(),
		/** Attribution weight in basis points, where 10000 = 100% credit. */
		attributionWeightBps: integer("attribution_weight_bps")
			.default(10000)
			.notNull(),
		revenueUsdMicros: bigint("revenue_usd_micros", { mode: "number" })
			.default(0)
			.notNull(),
		pipelineValueUsdMicros: bigint("pipeline_value_usd_micros", {
			mode: "number",
		})
			.default(0)
			.notNull(),
	...
	(table) => ({
		workspaceAgentOccurredIdx: index(
			"revenue_attribution_workspace_agent_occurred_idx",
		).on(table.workspaceId, table.agentId, table.occurredAt),
		agentOccurredIdx: index("revenue_attribution_agent_occurred_idx").on(
			table.agentId,
			table.occurredAt,
		),
		outcomeIdx: index("revenue_attribution_outcome_idx").on(
			table.outcomeId,
			table.outcomeType,
		),
		traceIdx: index("revenue_attribution_trace_idx").on(table.traceId),
	}),
);
