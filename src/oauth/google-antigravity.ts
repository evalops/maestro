/**
 * Google Antigravity OAuth integration (Gemini 3, Claude, GPT-OSS via Google Cloud).
 */
	...
	return new Promise((resolve, reject) => {
		...
		server.on("error", (err: NodeJS.ErrnoException) => {
			if (err.code === "EADDRINUSE") {
				reject(
					new Error(
						"Port 51121 is already in use. Please close the other process and try again.",
					),
				);
				return;
			}
...
async function discoverProject(
	...
	onStatus?.("Checking for existing Antigravity project...");
	...
	onStatus?.("Using default Antigravity project.");
...
export async function refreshGoogleAntigravityToken(
	refreshToken: string,
...
export async function loginGoogleAntigravity(
	...
	try {
		...
		logger.info("Google Antigravity OAuth login successful");
	} finally {
		server.close();
	}
}
