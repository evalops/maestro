...
export interface UserMessageItem {
	...
	text: string; // The message text (normalized to single line)
 ...
 */
class UserMessageList extends Container {
	private messages: UserMessageItem[] = [];
	private selectedIndex = 0;
	public onSelect?: (messageIndex: number) => void;
	public onCancel?: () => void;
	private maxVisible = 10; // Max messages visible
	...
	render(width: number): string[] {
		...
		if (this.messages.length === 0) {
			lines.push(chalk.dim("  No user messages found"));
			return lines;
		}
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			const truncatedMsg = message.text.substring(0, maxMsgWidth);
			const messageLine =
				cursor +
				(isSelected ? chalk.bold(truncatedMsg) : chalk.dim(truncatedMsg));
...
export class UserMessageSelectorComponent extends Container {
	...
	constructor(
		...
		this.addChild(
			new Text(
				chalk.dim("Select a message to create a new branch from that point"),
				1,
				0,
			),
		);
		...
		this.addChild(new Spacer(1));
		this.addChild(
			new Text(chalk.dim("↑/↓: Navigate  Enter: Select  Esc: Cancel"), 1, 0),
		);
